...
| 策略 | 标的 | 逻辑 |
|---|---|---|
| RSI+动量 共识（分歧各半） | 512800 银行 / 159949 创业板50 | RSI(14) 与 动量(20) 两信号周五比较：一致满仓该标的、分歧各持一半，次周一生效，**周五收盘执行**，永不空仓 |
...
- 数据源:
  - 策略1(RSI+动量 共识): 场内**前复权价格**(腾讯行情, `fetch_stock.py` 维护 `data/512800_股票.csv`、`data/159949_股票.csv`)，**周五收盘执行**口径
